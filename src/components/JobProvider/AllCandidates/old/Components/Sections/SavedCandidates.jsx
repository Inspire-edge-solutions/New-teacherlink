...
const FULL_API = 'https://xx22er5s34.execute-api.ap-south-1.amazonaws.com/dev/change';
...
const SavedCandidates = () => {
  ...
  // --- MAIN DATA FETCH (Saved only for this user) ---
  useEffect(() => {
    if (!user) return;
    ...
    const fetchData = async () => {
      try {
        ...
        const { data: savedRows } = await axios.get(FAV_API);
        const saveds = Array.isArray(savedRows) ? savedRows : [];

        // Only saved for this user
        const savedUids = saveds
          .filter(f =>
        ...
        // Favourites for this user (for icon UI)
        const favUids = saveds
          .filter(f =>
        ...
        // Downloaded for this user (for UI)
        const downloadedUids = saveds
          .filter(f =>
        ...
        // 2. Fetch all profiles, then filter by those UIDs
        const { data: profilesRaw } = await axios.get(FULL_API);
        
        // Handle case where response is an array containing arrays
        let profiles = profilesRaw;
        if (Array.isArray(profilesRaw) && profilesRaw.length > 0 && Array.isArray(profilesRaw[0])) {
          profiles = profilesRaw[0]; // Take the first array which contains the actual candidates
        }
        
        // Ensure profiles is an array of objects
        if (!Array.isArray(profiles)) {
          console.error('Expected array but got:', typeof profiles);
          profiles = [];
        }
        
        setAllProfiles(profiles);

        const onlySaved = profiles.filter(p => savedUids.includes(p.firebase_uid));
        // Decode the candidates data
        const decodedSaved = decodeCandidatesData(onlySaved);
        setSavedCandidates(decodedSaved);
        setFiltered(decodedSaved);
        setHasNoResults(onlySaved.length === 0);
      } catch (error) {
  ...
  const postOrPutUserFeature = async (candidateId, updatePayload) => {
    ...
    try {
      ...
      // Always refresh all three for UI
      ...
      // If unsave, re-filter for visible list
      if (updatePayload.saved_candidate === 0) {
        const onlySaved = allProfiles.filter(p =>
          afterUpdate.some(f =>
      ...
      if (updatePayload.saved_candidate === 1) {
        // If just saved, re-filter in case new ones added
        const onlySaved = allProfiles.filter(p =>
          afterUpdate.some(f =>
            f.firebase_uid === p.firebase_uid &&
            f.added_by === currentUserUid &&
  ...
  useEffect(() => {
    // Keep select-all in sync when page or selection changes
    const pageIds = new Set(currentCandidates.map(c => c.firebase_uid));
  ...
  const handleSearch = useCallback((searchTerm) => {
    if (!searchTerm) {
      ...
      setFiltered(savedCandidates);
      return;
    }
    setIsSearching(true);
    const results = searchCandidates(savedCandidates, searchTerm);
  ...
  if (!user) {
    return (
      <div className="no-results-message alert alert-danger">
        Please log in to view saved candidates.
      </div>
    );
  }
    ...
    <div className="saved-candidates-container">
      <div className="widget-title d-flex justify-content-between align-items-center">
        <div className="title-area">
          <h4 className="text-2xl font-semibold bg-gradient-brand bg-clip-text text-transparent m-0">Saved Candidates <span className="badge bg-primary">{filtered.length}</span></h4>
        </div>
