...
const UnlockedCandidates = () => {
  ...
  useEffect(() => {
    ...
    const fetchCandidates = async () => {
      try {
        const { data: dataRaw } = await axios.get(API);
        
        // Handle case where response is an array containing arrays
        let data = dataRaw;
        if (Array.isArray(dataRaw) && dataRaw.length > 0 && Array.isArray(dataRaw[0])) {
          data = dataRaw[0]; // Take the first array which contains the actual candidates
        }
        ...
        console.log('Unlocked candidate IDs:', unlockedCandidateIds);
        const unlockedCandidates = data.filter(candidate => 
          unlockedCandidateIds.includes(candidate.firebase_uid)
        );
        console.log('Unlocked candidates:', unlockedCandidates);
        // Decode the candidates data
        //const decodedCandidates = decodeCandidatesData(unlockedCandidates);
        setAll(unlockedCandidates);
        setFiltered(unlockedCandidates);
