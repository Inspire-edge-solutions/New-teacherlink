...
const ConfirmationModal = ({ 
  ...
  return createPortal(
    <div 
      className="fixed inset-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center z-[9999]"
      onClick={(e) => {
    ...
    >
      <div 
        className="bg-white rounded-lg p-5 max-w-md w-[90%] shadow-xl relative animate-modalSlideIn"
      ...
      >
        <div className="flex justify-between items-center mb-4">
          ...
          <button 
            type="button" 
            className="bg-none border-none text-xl cursor-pointer text-gray-600 p-0 w-5 h-5 hover:bg-gray-100 rounded-full flex items-center justify-center transition-colors"
        ...
        <div className="flex justify-end gap-2.5">
          <button 
            type="button" 
            className="px-4 py-2 border border-gray-300 rounded bg-white text-gray-600 cursor-pointer text-sm hover:bg-gray-50 transition-colors"
            onClick={() => setShowConfirmDialog(false)}
          >
            Cancel
          </button>
          <button 
            type="button" 
            className="px-4 py-2 border-none rounded bg-gradient-brand text-white cursor-pointer text-sm hover:opacity-90 transition-opacity"
...
const ActiveJobs = ({ 
  ...
  const getStatusBadge = (job) => {
    ...
    if (isJobClosed(job)) {
      return <span className="px-2 py-1 rounded text-xs font-medium uppercase tracking-wide bg-gray-200 text-gray-600 border border-gray-300">Closed</span>;
    }
    
    const daysRemaining = getDaysRemaining(job);
    if (daysRemaining > 7) {
      return <span className="px-2 py-1 rounded text-xs font-medium uppercase tracking-wide bg-green-100 text-green-800 border border-green-200">Active ({daysRemaining} days)</span>;
    } else if (daysRemaining > 0) {
      return <span className="px-2 py-1 rounded text-xs font-medium uppercase tracking-wide bg-yellow-100 text-yellow-800 border border-yellow-200">Expiring Soon ({daysRemaining} days)</span>;
    } else {
      return <span className="px-2 py-1 rounded text-xs font-medium uppercase tracking-wide bg-red-100 text-red-800 border border-red-200">Expired</span>;
    }
  };
    ...
    return (
      <div className="w-full max-w-full">
        <div className="text-center py-12">
          ...
          <p className="mt-2 text-gray-600">Loading active jobs...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="p-0 w-full max-w-full">
          {jobs.length === 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <h6 className="text-gray-500 text-lg font-semibold mb-2">No Jobs Found</h6>
            ...
            <div className="w-full">
              <div className="flex justify-between items-center mb-5 flex-wrap gap-4 w-full">
                <h6 className="text-gray-700 text-base font-semibold">Active & Closed Jobs: {filteredJobs.length}</h6>
              ...
              {filteredJobs.map((job, index) => (
                <div key={job.id || index} className={`mb-4 border border-gray-200 rounded-lg shadow-sm transition-all duration-300 overflow-hidden bg-white p-4 w-full hover:shadow-md hover:-translate-y-0.5 hover:border-gray-300 ${isJobClosed(job) ? 'opacity-70 bg-gray-50 border-gray-200 relative' : ''}`}>
                  {isJobClosed(job) && (
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              ...
                              <div className="text-sm leading-relaxed text-gray-600">
                                ...
                                <div className="mb-2 py-1"><strong className="text-gray-900 font-semibold min-w-[120px] inline-block">Days Remaining:</strong> {getDaysRemaining(job)} days</div>
                              </div>
                            </div>
                            <div>
