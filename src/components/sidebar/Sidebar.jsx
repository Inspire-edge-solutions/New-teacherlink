                ...
                >
                  ...
                  {!isCollapsed && (
                    <span className={`${active ? "bg-gradient-to-r from-[#F34B58] to-[#A1025D] bg-clip-text text-transparent font-semibold" : "text-gray-700"} flex items-center gap-2`}>
                      {item.label}
                      {item.label === "Notifications" && unreadCount > 0 && (
                        <span className="bg-gradient-to-r from-[#F34B58] to-[#A1025D] text-white text-xs font-bold rounded-full min-w-[20px] h-5 flex items-center justify-center px-1.5">
                          {unreadCount > 99 ? '99+' : unreadCount}
                        </span>
                      )}
                    </span>
