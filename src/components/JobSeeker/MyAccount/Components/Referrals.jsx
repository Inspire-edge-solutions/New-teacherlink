...
const Referrals = ({ user, onSuccess }) => {
  ...
  const handleAddContact = async (e) => {
    ...
    try {
      ...
      for (let i = newContacts.length + 1; i <= 20; i++) {
        body[`contact${i}`] = null;
      }
      ...
      if (!addResponse.ok) throw new Error("Failed to add candidate");
      ...
      toast.error("Network error adding candidate. Please try again.");
    }
  };
  ...
  const handleEditSave = async (idx) => {
    ...
    try {
      ...
      for (let i = updatedContacts.length + 1; i <= 20; i++) {
        body[`contact${i}`] = null;
      }
      ...
      toast.error("Network error updating candidate. Please try again.");
    }
  };
  ...
  // Add this function to handle the submit-all-candidates button
  const handleSubmitReferrals = (event) => {
    event.preventDefault();
    toast.info("Submit all candidates logic not implemented yet.");
  };
  ...
  return (
    <div className="w-full">
      <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 lg:p-8">
        {/* Header */}
        <div className="text-center mb-6 sm:mb-8">
          <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-800 mb-2">Refer candidates and get free access to our Basic Plan</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            {contacts.length > 0 && (
              ...
              >
                {contacts.length < 20
                  ? `Add ${20 - contacts.length} more candidate${20 - contacts.length === 1 ? '' : 's'} to submit`
